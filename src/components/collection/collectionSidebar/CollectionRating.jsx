...
const CollectionRating = ({ filter, setFilter, attributeAPIData }) => {
  ...
  const [category, subcategory, attribute, price, sortBy, field, layout] = useCustomSearchParams(['category', 'subcategory', 'attribute', 'price', 'sortBy', 'field', 'layout']);
  ...
  const applyRating = (event) => {
    ...
    if (temp.length > 0) {
      const queryParams = new URLSearchParams({ ...category, ...subcategory, ...attribute, ...price, ...sortBy, ...field, ...layout, rating: temp }).toString();
      router.push(`${pathname}?${queryParams}`);
    } else {
      const queryParams = new URLSearchParams({ ...category, ...subcategory, ...attribute, ...price, ...sortBy, ...field, ...layout }).toString();
      router.push(`${pathname}?${queryParams}`);
    }
  };
  return (
