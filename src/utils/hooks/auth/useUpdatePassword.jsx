...
const useUpdatePassword = () => {
  ...
  return useMutation({
    mutationFn: async (data) => {
      ...
      if (!resetToken) {
        throw new Error('Reset token is missing. Please request a new password reset.');
      }
    ...
    onSuccess: (resData) => {
      if (resData.status === 200 || resData.status === 201) {
        const successMessage = resData.data.message || 'Your password has been changed successfully. Use your new password to log in.';
        ...
        ToastNotification('error', resData.data?.message || 'Failed to update password');
      }
    },
    onError: (error) => {
